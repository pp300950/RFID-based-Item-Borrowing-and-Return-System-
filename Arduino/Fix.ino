...
bool p_errorShowThai = false; // เก็บสถานะก่อนหน้าเพื่อรู้ว่าต้อง redraw 
...
void setup() {
  ...
  tft.setAttribute(UTF8_SWITCH, true); // เปิด UTF-8 สำหรับข้อความไทย
  tft.loadFont("Thai20", LittleFS);    // โหลดฟอนต์ไทยไว้ล่วงหน้า (ใช้เฉพาะตอนสลับเป็นไทย)
  ...
  if (pressed) {
    ...
    if (currentMode == 0) {
      ...
      if (currentSubMode == "OUTDOOR") {
        if (real_x >= 0 && real_x <= 480 && real_y >= 210 && real_y <= 248) tappedErrorRow = true;
      } else {
        if (real_x >= 0 && real_x <= 480 && real_y >= 265 && real_y <= 320) tappedErrorRow = true;
      }

      if (tappedErrorRow) {
        beep();
        errorShowThai = !errorShowThai;
        p_ac_error_code = "__FORCE__"; // บังคับให้ redraw บรรทัด error รอบถัดไป
        delay(150);
        return;
      }
  ...
  if (brandIndex == 0) { // DAIKIN
    ...
    if (bitCount >= 64) {  
      ...
      if (currentAmp > AMP_LIMIT) {  
        ...
        switch (statusByte) {  
          case 0: diagnostic = (hz > 0) ? "COMPRESSOR RUNNING" : "READY"; break;
          case 1: errorDisplay = "E15"; diagnostic = "Communication Error"; break;
          case 2: errorDisplay = "F1"; diagnostic = "IPM Module Protection "; break;
          case 3: errorDisplay = "F22"; diagnostic = "AC Current Overload "; break;
          case 4: errorDisplay = "F11"; diagnostic = "Compressor Sync Out"; break;
          case 5: errorDisplay = "F3"; diagnostic = "Internal Comm Error"; break;
          case 6: errorDisplay = "F20"; diagnostic = "Voltage Abnormal"; break;
          case 7: errorDisplay = "F12"; diagnostic = "Outdoor EEPROM Error"; break;
          case 8: errorDisplay = "F4"; diagnostic = "Exhaust Temp Too High"; break;
          case 9: errorDisplay = "F8"; diagnostic = "Outdoor Fan Motor Error "; break;
          case 10: errorDisplay = "F21"; diagnostic = "Defrost Sensor Error"; break;
          case 12: errorDisplay = "F6"; diagnostic = "Ambient Sensor Fault"; break;
          case 13: errorDisplay = "F23"; diagnostic = "DC Bus Overcurrent"; break;
          case 18: errorDisplay = "F2"; diagnostic = "Compressor Drive Abnormal "; break;
          case 19: errorDisplay = "F28"; diagnostic = "Compressor Startup Fail"; break;
          case 24: errorDisplay = "F25"; diagnostic = "Current Detection Circuit Error "; break;
          case 35: errorDisplay = "E7"; diagnostic = "Indoor/Outdoor Communication Fault"; break;
          case 48: errorDisplay = "FC"; diagnostic = "Driver Board Error"; break;
          default:  
            if (hz > 0) diagnostic = "COMPRESSOR RUNNING";  
            break;  
        }  
      }  
      ...
      if (errorDisplay == "--" || errorDisplay == "00") {
        ...
        ac_error_code_raw = errorDisplay;
...
// สลับไปใช้ฟอนต์ไทยที่โหลดจาก LittleFS (Thai20.vlw)
// ใช้เฉพาะตอนวาดคำอธิบาย error เป็นภาษาไทยเท่านั้น จุดอื่นในโปรแกรมยังคง setFreeFont แบบเดิม
void setThaiFont() {
  tft.setTextFont(1); 
  tft.setTextSize(1);
  // หลังจาก loadFont("Thai20", LittleFS) ใน setup() แล้ว
  // เมื่อไม่ได้ set free font ค้างอยู่ ระบบจะใช้ฟอนต์ Thai20 นี้แทน
}
...
void updateMonitoringValues() {
  ...
  if (currentSubMode == "OUTDOOR") {
    ...
    if (errorShowThai) setThaiFont(); else tft.setFreeFont(&FreeSansBold9pt7b);
    drawValIfChanged(p_ac_error_code, ac_error_code, 165, 229);
    tft.setFreeFont(&FreeSansBold9pt7b); // คืนฟอนต์เดิมให้จุดอื่นที่วาดถัดไปในฟังก์ชันนี้
    tft.setTextPadding(0);
  } 
  else { 
    tft.setTextColor(valColor, bgColor);
    ...
    if (p_ac_error_code == "__FORCE__") p_ac_error_code = "";
    if (errorShowThai) setThaiFont(); else tft.setFreeFont(&FreeSansBold9pt7b);
    drawValIfChanged(p_ac_error_code, ac_error_code, 165, 285);
    tft.setFreeFont(&FreeSansBold9pt7b); // คืนฟอนต์เดิมให้จุดอื่นที่วาดถัดไปในฟังก์ชันนี้
    tft.setTextPadding(0);
  }
}
