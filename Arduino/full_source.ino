...
String getDaikinErrorDesc(String code) {
  if(code == "00") return "Normal / ทำงานปกติ";
  if(code == "A1") return "Indoor PCB defect / บอร์ดคอยล์เย็นเสีย";
  if(code == "A3") return "Drain pump abnormal / ปั๊มน้ำทิ้งผิดปกติ";
  if(code == "A5") return "Antifreeze / ป้องกันน้ำแข็งเกาะ";
  if(code == "A6") return "Indoor fan motor / พัดลมคอยล์เย็นเสีย";
  if(code == "A7") return "Swing motor error / มอเตอร์สวิงผิดปกติ";
  if(code == "A9") return "EEV error / วาล์ว EEV ผิดปกติ";
  if(code == "AF") return "Drain level over limit / น้ำทิ้งล้น";
  if(code == "C4") return "Indoor thermistor / เซนเซอร์คอยล์เย็นเสีย";
  if(code == "C5") return "Gas pipe thermistor / เซนเซอร์ท่อแก๊สเสีย";
  if(code == "C7") return "Louver limit switch / สวิตช์บานสวิงเสีย";
  if(code == "C9") return "Room temp sensor / เซนเซอร์อุณหภูมิห้อง";
  if(code == "CJ") return "Remote thermistor / เซนเซอร์รีโมทผิดปกติ";
  if(code == "U0") return "Shortage refrigerant / น้ำยาแอร์ขาด";
  if(code == "U2") return "Over-voltage or drop / แรงดันไฟตก-เกิน";
  if(code == "U4") return "Comm Error / สื่อสารคอยล์เย็น-ร้อนผิดปกติ";
  if(code == "U5") return "Comm Error / สื่อสารรีโมทผิดปกติ";
  if(code == "U7") return "Comm Error / สื่อสารคอยล์ร้อนผิดปกติ";
  if(code == "U8") return "Comm Error / สื่อสารรีโมทหลัก-รอง";
  if(code == "U9") return "Comm Error / สื่อสารระบบผิดปกติ";
  if(code == "UA") return "Field setting error / ตั้งค่าระบบผิดพลาด";
  if(code == "UC") return "Address setting / ตั้งค่า Address ผิด";
  if(code == "UH") return "System malfunction / ระบบทำงานผิดพลาด";
  if(code == "E1") return "Outdoor PCB error / บอร์ดคอยล์ร้อนเสีย";
  if(code == "E3") return "High pressure / แรงดันน้ำยาสูงเกิน";
  if(code == "E4") return "Low pressure / แรงดันน้ำยาต่ำเกิน";
  if(code == "E5") return "Comp overload / คอมเพรสเซอร์โหลดเกิน";
  if(code == "E6") return "Comp overcurrent / กระแสคอมเพรสเซอร์เกิน";
  if(code == "E7") return "Outdoor fan motor / พัดลมคอยล์ร้อนเสีย";
  if(code == "E8") return "Overcurrent (input) / กระแสไฟเข้าเกิน";
  if(code == "F3") return "Discharge temp / อุณภูมิท่อส่งสูงเกิน";
  if(code == "F6") return "High press control / แรงดันขณะทำความเย็นสูง";
  if(code == "H0") return "Compressor sensor fault / เซนเชอร์ระบบคอมเพรสเซอร์ขัดข้อง";
  if(code == "H3") return "High press switch / สวิตช์แรงดันสูงผิดปกติ";
  if(code == "H6") return "Comp position err / ตรวจจับตำแหน่งคอมฯ ผิดพลาด";
  if(code == "H7") return "Outdoor fan signal / สัญญาณพัดลมคอยล์ร้อนเสีย";
  if(code == "H8") return "CT error / เซนเซอร์กระแสไฟผิดปกติ";
  if(code == "H9") return "Outdoor air thermistor / เซนเซอร์อุณหภูมิภายนอก";
  if(code == "J3") return "Discharge thermistor / เซนเซอร์ท่อส่งเสีย";
  if(code == "J5") return "Suction thermistor / เซนเซอร์ท่อดูดเสีย";
  if(code == "J6") return "Outdoor coil thermistor / เซนเซอร์คอยล์ร้อนเสีย";
  if(code == "J8") return "Liquid pipe thermistor / เซนเซอร์ท่อของเหลวเสีย";
  if(code == "L1") return "Inverter PCB error / บอร์ดอินเวอร์เตอร์เสีย";
  if(code == "L3") return "Control box temp / อุณหภูมิกล่องคอนโทรลสูง";
  if(code == "L4") return "Heat sink temp / ฮีตซิงก์ร้อนเกิน";
  if(code == "L5") return "Inv comp overcurrent / กระแสคอมฯ อินเวอร์เตอร์เกิน";
  if(code == "L8") return "Inv comp overload / คอมฯ อินเวอร์เตอร์โหลดเกิน";
  if(code == "L9") return "Comp lock / คอมเพรสเซอร์ล็อค";
  if(code == "LC") return "Comm Error / สื่อสารอินเวอร์เตอร์-คอยล์ร้อนเสีย";
  if(code == "P1") return "Inv voltage / แรงดันไฟอินเวอร์เตอร์ไม่สมดุล";
  if(code == "P4") return "Heat sink thermistor / เซนเซอร์ฮีตซิงก์ผิดปกติ";
  if(code == "PJ") return "Capacity setting / ตั้งค่าขนาดความจุผิดพลาด";
  return "Unknown / รหัสไม่ตรงกับการสื่อสาร";
}
...
void setup() {
  ...
  // tft.loadFont("ThaiFont16", LittleFS); // [เพิ่ม] โหลดไฟล์ฟอนต์ภาษาไทย .vlw จาก Flash (ถ้ามีไฟล์ฟอนต์ใน LittleFS)
...
void loop() {
  ...
  if (currentMode == 30) {
    ...
    if (millis() - lastSignalReceived > 15000) {
      ac_error_code = "Comm Error / สัญญาณสื่อสารผิดพลาด";
    }
...
void handleAirconCommunication(int brandIndex, String mode) {
  if (brandIndex == 0) { // DAIKIN
    ...
    if (lastCompFreq > 0) {
      ac_status = "ON / เปิด";
      ac_in_status = "ON / เปิด";
    } else {
      ac_status = "OFF / ปิด";
      ac_in_status = "OFF / ปิด";
    }
    ...
    if (bitCount >= 64) {  
      ...
      String sysMode = "OFF / ปิด";  
      String errorDisplay = "--";  
      String diagnostic = "";  
        
      if (currentAmp > AMP_LIMIT) {  
        errorDisplay = "F2";  
        diagnostic = "OVERCURRENT PROTECTION / ป้องกันกระแสเกิน";  
      }  
      else if (statusByte == 19) {  
        errorDisplay = "F28";  
        diagnostic = "COMPRESSOR STARTUP FAIL / คอมฯ สตาร์ทล้มเหลว";  
      }  
      else if (statusByte == 9) {  
        errorDisplay = "F8";  
        diagnostic = "OUTDOOR FAN ERROR / พัดลมคอยล์ร้อนขัดข้อง";  
      }  
        ...
        switch (statusByte) {  
          case 0: diagnostic = (hz > 0) ? "RUNNING / ทำงานอยู่" : "READY / พร้อมทำงาน"; break;
          case 1: errorDisplay = "E15"; diagnostic = "Communication Error / การสื่อสารผิดพลาด"; break;
          case 2: errorDisplay = "F1"; diagnostic = "IPM Protection / ระบบไอพีเอ็มป้องกัน"; break;
          case 3: errorDisplay = "F22"; diagnostic = "AC Overload / กระแสไฟเกิน"; break;
          case 4: errorDisplay = "F11"; diagnostic = "Compressor Sync Out / คอมฯ หลุดซิงก์"; break;
          case 5: errorDisplay = "F3"; diagnostic = "Internal Comm Error / สื่อสารภายในผิดพลาด"; break;
          case 6: errorDisplay = "F20"; diagnostic = "Voltage Abnormal / แรงดันไฟผิดปกติ"; break;
          case 7: errorDisplay = "F12"; diagnostic = "Outdoor EEPROM Error / อีพรอมคอยล์ร้อนเสีย"; break;
          case 8: errorDisplay = "F4"; diagnostic = "Exhaust Temp High / อุณหภูมิท่อส่งสูงเกิน"; break;
          case 9: errorDisplay = "F8"; diagnostic = "Outdoor Fan Error / พัดลมคอยล์ร้อนเสีย"; break;
          case 10: errorDisplay = "F21"; diagnostic = "Defrost Sensor Error / เซนเซอร์ละลายน้ำแข็งเสีย"; break;
          case 12: errorDisplay = "F6"; diagnostic = "Ambient Sensor Fault / เซนเซอร์อุณหภูมิแวดล้อมเสีย"; break;
          case 13: errorDisplay = "F23"; diagnostic = "DC Bus Overcurrent / กระแสกระแสตรงเกิน"; break;
          case 18: errorDisplay = "F2"; diagnostic = "Compressor Drive Abnormal / ระบบขับคอมฯ ผิดปกติ"; break;
          case 19: errorDisplay = "F28"; diagnostic = "Compressor Startup Fail / คอมฯ สตาร์ทล้มเหลว"; break;
          case 24: errorDisplay = "F25"; diagnostic = "Current Detection Error / วงจรตรวจวัดกระแสเสีย"; break;
          case 35: errorDisplay = "E7"; diagnostic = "Comm Fault / สื่อสารคอยล์เย็น-ร้อนเสีย"; break;
          case 48: errorDisplay = "FC"; diagnostic = "Driver Board Error / บอร์ดไดรเวอร์เสีย"; break;
          default:  
            if (hz > 0) diagnostic = "RUNNING / ทำงานอยู่";  
            break;  
        }  
      }  
        
      sysMode = (hz > 0) ? "ON / เปิด" : "OFF / ปิด";  
...
void drawSplashScreen() {
  ...
  tft.drawString("INITIALIZING SYSTEM / กำลังเริ่มต้นระบบ...", cx, 258);
  ...
  tft.setTextColor(ICE_CYAN);
  tft.drawString("SYSTEM READY / ระบบพร้อมใช้งาน", cx, 255);
  tft.setTextColor(TEXT_MUTED);
  tft.drawString("- TAP SCREEN TO START / แตะหน้าจอเพื่อเริ่มต้น -", cx, 280);
...
void drawBrandScreen(int brandIndex) {
  ...
  tft.setFreeFont(&FreeSansBold9pt7b);
  tft.drawString("Please select mode / เลือกระบบการทำงาน:", 240, 120);
...
void drawMonitoringScreen(int brandIndex, String modeTitle) {
  ...
  if (modeTitle == "OUTDOOR") {
    ...
    tft.setTextColor(COLOR_TEXT);
    tft.drawString("ERROR CODE / รหัสผิดปกติ :", 10, 229);
    ...
    tft.drawString("ERROR CODE / รหัสผิดปกติ :", 10, 285);
  }
}
