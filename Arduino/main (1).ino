...
#include <SPI.h>
#include <TFT_eSPI.h>
#include <LittleFS.h> // [เพิ่ม] สำหรับโหลดไฟล์ฟอนต์ภาษาไทย .vlw
...
String calculatePressH(float temp) {
  ...
  for (int i = 0; i < PRESS_TABLE_SIZE - 1; i++) {
    if (temp >= pressLookupTable[i].temp && temp <= pressLookupTable[i+1].temp) {
      ...
      return String((int)(interpolated + 0.5)); 
    }
  }
  return "";
}
...
void setup() {
  ...
  // [เพิ่ม] เริ่มทำงาน LittleFS และเปิดโหมดอ่าน UTF-8 ภาษาไทย
  ...
  tft.setAttribute(UTF8_SWITCH, true); // เปิดใช้งาน UTF-8 สำหรับภาษาไทย
  tft.loadFont("Thai20", LittleFS);    // โหลดฟอนต์ไทยจาก LittleFS
...
void loop() {
  ...
  if (pressed) {
    ...
    if (currentMode == 0) {
      ...
      if (clickedGrid != -1) {
        ...
        if (clickedGrid == 8) {
          String btnText = (currentPage == 0) ? "หน้า 2 ->" : (currentPage == 1 ? "หน้า 3 ->" : "<- หน้า 1");
      ...
      if (real_x >= backBtnX && real_x <= (backBtnX + backBtnW) && real_y >= backBtnY && real_y <= (backBtnY + backBtnH + 5)) {
        draw3DButton(backBtnX, backBtnY, backBtnW, backBtnH, bgCol, bgCol, "กลับ", true, isBlack);
      ...
      if (real_x >= subBtnX_1 && real_x <= (subBtnX_1 + subBtnW) && real_y >= subBtnY && real_y <= (subBtnY + subBtnH + 5)) {
        draw3DButton(subBtnX_1, subBtnY, subBtnW, subBtnH, bgCol, bgCol, "คอยล์เย็น / คอยล์ร้อน", true, isBlack);
        beep(); delay(150); 
      ...
      if (real_x >= subBtnX_2 && real_x <= (subBtnX_2 + subBtnW) && real_y >= subBtnY && real_y <= (subBtnY + subBtnH + 5)) {
        draw3DButton(subBtnX_2, subBtnY, subBtnW, subBtnH, bgCol, bgCol, "คอยล์ร้อน", true, isBlack);
        beep(); delay(150); 
      ...
      if (real_x >= 40 && real_x <= 230 && real_y >= 235 && real_y <= 285) {
        draw3DButton(40, 235, 190, 50, bgCol, bgCol, "หน้าจอตรวจสอบ", true, isBlack);
        beep(); delay(150); 
        currentMode = 30;
      ...
      if (real_x >= 250 && real_x <= 440 && real_y >= 235 && real_y <= 285) {
        draw3DButton(250, 235, 190, 50, bgCol, bgCol, "ออสซิลโลสโคป", true, isBlack);
        beep(); delay(150); 
        currentMode = 40;
      ...
      if (currentSubMode == "OUTDOOR") {
        if (real_y >= 240) { 
          if (real_x < 160) {
            draw3DButton(colX[0], 255, btnW, 55, bgCol, COLOR_BG, "เริ่ม", true, isBlack);
            beep(); delay(120);
            draw3DButton(colX[0], 255, btnW, 55, bgCol, COLOR_BG, "เริ่ม", false, isBlack);
            sendAirconCommand(selectedBrand, "START");
            return;
          } 
          else if (real_x >= 160 && real_x < 320) {
            draw3DButton(colX[1], 255, btnW, 55, bgCol, COLOR_BG, "เพิ่มความถี่", true, isBlack);
            beep(); delay(120);
            draw3DButton(colX[1], 255, btnW, 55, bgCol, COLOR_BG, "เพิ่มความถี่", false, isBlack);
            sendAirconCommand(selectedBrand, "FREQ+");
            return;
          } 
          else if (real_x >= 320) {
            draw3DButton(colX[2], 255, btnW, 55, bgCol, COLOR_BG, "หยุด", true, isBlack);
            beep(); delay(120);
            draw3DButton(colX[2], 255, btnW, 55, bgCol, COLOR_BG, "หยุด", false, isBlack);
      ...
      if (real_y >= 260 && real_y <= 310) {
        if (real_x >= 8 && real_x <= 88) {
          ...
          draw3DButton(392, 260, 80, 50, isScopeRunning ? tft.color565(0, 180, 0) : tft.color565(180, 0, 0), tft.color565(0, 0, 0), isScopeRunning ? "ทำงาน" : "หยุด", false, false);
        }
      }
    }
  }
  ...
  if (currentMode == 30) {
    ...
    if (millis() - lastSignalReceived > 15000) {
      ac_error_code = "สัญญาณสื่อสารผิดพลาด";
    }
...
void handleAirconCommunication(int brandIndex, String mode) {
  if (brandIndex == 0) { // DAIKIN
    ...
    if (lastErrorCode == "00" || lastErrorCode == "") {
      ...
      ac_error_code = lastErrorCode + " (" + getDaikinErrorDesc(lastErrorCode) + ")";
    }
  } 
    ...
    if (bitCount >= 64) {  
      ...
      if (currentAmp > AMP_LIMIT) {  
        errorDisplay = "F2";  
        diagnostic = "ป้องกันกระแสเกิน";  
      }  
      else if (statusByte == 19) {  
        errorDisplay = "F28";  
        diagnostic = "คอมฯ สตาร์ทล้มเหลว";  
      }  
      else if (statusByte == 9) {  
        errorDisplay = "F8";  
        diagnostic = "พัดลมคอยล์ร้อนขัดข้อง";  
      }  
      else if ((statusByte == 4 || statusByte == 2) && hz == 0 && currentAmp == 0) {  
        errorDisplay = "F1";  
        diagnostic = "IPM FAIL / NO DRIVE / ไอพีเอ็มขัดข้อง";  
      }  
      else {  
        switch (statusByte) {  
          case 0: diagnostic = (hz > 0) ? "ทำงานอยู่" : "พร้อมทำงาน"; break;
          case 1: errorDisplay = "E15"; diagnostic = "การสื่อสารผิดพลาด"; break;
          case 2: errorDisplay = "F1"; diagnostic = "ระบบไอพีเอ็มป้องกัน"; break;
          case 3: errorDisplay = "F22"; diagnostic = "กระแสไฟเกิน"; break;
          case 4: errorDisplay = "F11"; diagnostic = "คอมฯ หลุดซิงก์"; break;
          case 5: errorDisplay = "F3"; diagnostic = "สื่อสารภายในผิดพลาด"; break;
          case 6: errorDisplay = "F20"; diagnostic = "แรงดันไฟผิดปกติ"; break;
          case 7: errorDisplay = "F12"; diagnostic = "อีพรอมคอยล์ร้อนเสีย"; break;
          case 8: errorDisplay = "F4"; diagnostic = "อุณหภูมิท่อส่งสูงเกิน"; break;
          case 9: errorDisplay = "F8"; diagnostic = "พัดลมคอยล์ร้อนเสีย"; break;
          case 10: errorDisplay = "F21"; diagnostic = "เซนเซอร์ละลายน้ำแข็งเสีย"; break;
          case 12: errorDisplay = "F6"; diagnostic = "เซนเซอร์อุณหภูมิแวดล้อมเสีย"; break;
          case 13: errorDisplay = "F23"; diagnostic = "กระแสกระแสตรงเกิน"; break;
          case 18: errorDisplay = "F2"; diagnostic = "ระบบขับคอมฯ ผิดปกติ"; break;
          case 19: errorDisplay = "F28"; diagnostic = "คอมฯ สตาร์ทล้มเหลว"; break;
          case 24: errorDisplay = "F25"; diagnostic = "วงจรตรวจวัดกระแสเสีย"; break;
          case 35: errorDisplay = "E7"; diagnostic = "สื่อสารคอยล์เย็น-ร้อนเสีย"; break;
          case 48: errorDisplay = "FC"; diagnostic = "บอร์ดไดรเวอร์เสีย"; break;
          default:  
            if (hz > 0) diagnostic = "ทำงานอยู่";  
            break;  
        }  
      }  
        
      sysMode = (hz > 0) ? "เปิด" : "ปิด";  
      ...
      if (errorDisplay == "--" || errorDisplay == "00") {
        ...
        ac_error_code = errorDisplay + " : " + diagnostic; 
      }
    }
  }
  else {
...
void drawSplashScreen() {
  ...
  tft.drawString("เครื่องทดสอบอินเวอร์เตอร์อัจฉริยะ", cx, 192);

  setThaiFont();
  tft.setTextColor(TEXT_MUTED);
  tft.drawString("เครื่องมือวินิจฉัยเครื่องปรับอากาศ", cx, 215);

  int barX = 100, barY = 265, barW = 280, barH = 10;
  tft.setTextDatum(BC_DATUM);
  tft.drawString("กำลังเริ่มต้นระบบ...", cx, 258);
  ...
  tft.setTextColor(ICE_CYAN);
  tft.drawString("ระบบพร้อมใช้งาน", cx, 255);
  tft.setTextColor(TEXT_MUTED);
  tft.drawString("- แตะหน้าจอเพื่อเริ่มต้น -", cx, 280);
...
// [เพิ่ม] สลับไปใช้ฟอนต์ไทยที่โหลดจาก LittleFS (Thai20.vlw) แทน setFreeFont
// เรียกฟังก์ชันนี้แทนทุกจุดที่เคยเรียก tft.setFreeFont(&FreeSansXXX)
void setThaiFont() {
  tft.setTextFont(1);     // สลับกลับไปใช้ระบบฟอนต์ GFX/Smooth ปกติก่อน (เคลียร์ state ของ Free Font)
  tft.setTextFont(1);
  tft.setTextSize(1);
  // การเรียก loadFont("Thai20", LittleFS) ใน setup() จะทำให้ฟอนต์นี้ถูกใช้เป็นค่าเริ่มต้น
  // เมื่อไม่ได้เรียก setFreeFont ค้างอยู่ ระบบจะวาดด้วยฟอนต์ Thai20 ที่โหลดไว้โดยอัตโนมัติ
}
...
void drawMainMenu() {
  ...
  setThaiFont();
  tft.drawString("เลือกยี่ห้อ (" + String(currentPage + 1) + "/3)", 240, 20);

  for (int i = 0; i < 9; i++) {
    int c = i % 3, r = i / 3;
    if (i == 8) {
      if (currentPage == 0) draw3DButton(colX[c], rowY[r], btnW, btnH, tft.color565(120, 130, 140), MY_BG, "หน้า 2 ->", false, false);
      else if (currentPage == 1) draw3DButton(colX[c], rowY[r], btnW, btnH, tft.color565(120, 130, 140), MY_BG, "หน้า 3 ->", false, false);
      else draw3DButton(colX[c], rowY[r], btnW, btnH, tft.color565(120, 130, 140), MY_BG, "<- หน้า 1", false, false);
    } else {
...
void drawBrandScreen(int brandIndex) {
  ...
  setThaiFont();
  tft.drawString("กรุณาเลือกระบบการทำงาน:", 240, 120);
  
  draw3DButton(backBtnX, backBtnY, backBtnW, backBtnH, bgColor, bgColor, "กลับ", false, isBlack);
  draw3DButton(subBtnX_1, subBtnY, subBtnW, subBtnH, bgColor, bgColor, "คอยล์เย็น / คอยล์ร้อน", false, isBlack);
...
void drawDevScreen(int brandIndex) {
  ...
  setThaiFont();
  String devSubModeThai = (currentSubMode == "OUTDOOR") ? "คอยล์ร้อน" : "คอยล์เย็น / คอยล์ร้อน";
  tft.drawString("โหมด: " + devSubModeThai, 20, 70);
  ...
  tft.setTextColor(isBlack ? tft.color565(0, 0, 0) : tft.color565(255, 255, 255)); 
  setThaiFont();
  tft.drawString("ระบบพร้อมใช้งาน (แสดงผลสด)", 240, 140);

  tft.setTextColor(isBlack ? tft.color565(50, 50, 50) : tft.color565(200, 200, 200));
  setThaiFont();
  tft.drawString("เปิดใช้งานการรับสัญญาณสดและออสซิลโลสโคป", 240, 180);

  draw3DButton(40, 235, 190, 50, bgColor, bgColor, "หน้าจอตรวจสอบ", false, isBlack);
  draw3DButton(250, 235, 190, 50, bgColor, bgColor, "ออสซิลโลสโคป", false, isBlack);
}
...
void drawMonitoringScreen(int brandIndex, String modeTitle) {
  ...
  String subModeThai = (currentSubMode == "OUTDOOR") ? "คอยล์ร้อน" : "คอยล์เย็น / คอยล์ร้อน";
  String headerTxt = brandNames[brandIndex] + " | " + subModeThai;
  ...
  if (modeTitle == "OUTDOOR") {
    ...
    tft.setTextDatum(ML_DATUM);
    setThaiFont();
    tft.drawString(" OUTDOOR UNIT / คอยล์ร้อน", 5, 47);
    ...
    tft.drawString("สถานะ:", col1, r1); tft.drawString("ความถี่:", col2, r1);     tft.drawString("กระแส:", col3, r1);
    tft.drawString("อุณหภูมิออก:", col1, r2);  tft.drawString("คอยล์:", col2, r2);     tft.drawString("ท่อส่ง:", col3, r2);
    tft.drawString("IPM:", col1, r3);  tft.drawString("DC Bus:", col2, r3);   tft.drawString("EEV:", col3, r3);
    tft.drawString("พัดลม:", col1, r4);    tft.drawString("แรงดันต่ำ:", col2, r4);  tft.drawString("แรงดันสูง:", col3, r4);

    tft.fillRect(0, 210, 480, 38, COLOR_HEADER);
    tft.drawLine(0, 210, 480, 210, COLOR_LINE);
    tft.drawLine(0, 248, 480, 248, COLOR_LINE);
    tft.setTextColor(COLOR_TEXT);
    tft.drawString("รหัสผิดปกติ :", 10, 229);

    draw3DButton(colX[0], 255, btnW, 55, COLOR_BRAND, COLOR_BG, "เริ่ม", false, isBlack);
    draw3DButton(colX[1], 255, btnW, 55, COLOR_BRAND, COLOR_BG, "เพิ่มความถี่", false, isBlack);
    draw3DButton(colX[2], 255, btnW, 55, COLOR_BRAND, COLOR_BG, "หยุด", false, isBlack);
  } 
  else {
    tft.fillRect(0, 35, 480, 25, COLOR_HEADER);
    tft.setTextColor(COLOR_TEXT);
    ...
    tft.drawString("สถานะ:", col1, or1); tft.drawString("ความถี่:", col2, or1); tft.drawString("กระแส:", col3, or1);
    tft.drawString("อุณหภูมิออก:", col1, or2);  tft.drawString("คอยล์:", col2, or2); tft.drawString("ท่อส่ง:", col3, or2);
    tft.drawString("IPM:", col1, or3);  tft.drawString("DC Bus:", col2, or3); tft.drawString("EEV:", col3, or3);
    tft.drawString("พัดลม:", col1, or4);    tft.drawString("แรงดันต่ำ:", col2, or4); tft.drawString("แรงดันสูง:", col3, or4);

    tft.fillRect(0, 180, 480, 25, COLOR_HEADER);
    tft.setTextColor(COLOR_TEXT);
    setThaiFont();
    tft.drawString(" INDOOR UNIT / คอยล์เย็น", 5, 192);
    ...
    int ir1 = 220, ir2 = 250;
    tft.drawString("สถานะ:", col1, ir1); tft.drawString("รุ่น:", col2, ir1); tft.drawString("ตั้งอุณหภูมิ:", col3, ir1);
    tft.drawString("ห้อง:", col1, ir2);   tft.drawString("คอยล์:", col2, ir2);  tft.drawString("พัดลม:", col3, ir2);

    tft.fillRect(0, 265, 480, 55, COLOR_HEADER);
    tft.drawLine(0, 265, 480, 265, COLOR_LINE);
    tft.drawString("รหัสผิดปกติ :", 10, 285);
  }
}
...
void updateMonitoringValues() {
  ...
  if (currentSubMode == "OUTDOOR") {
    ...
    drawValIfChanged(p_ac_status, ac_status, col1 + 90, r1);
    drawValIfChanged(p_ac_freq, ac_freq, col2 + 85, r1);
    drawValIfChanged(p_ac_amp, ac_amp, col3 + 80, r1);

    drawValIfChanged(p_ac_out_t, ac_out_t, col1 + 110, r2);
    drawValIfChanged(p_ac_coil, ac_coil, col2 + 70, r2);
    drawValIfChanged(p_ac_disch, ac_disch, col3 + 70, r2);

    drawValIfChanged(p_ac_ipm_t, ac_ipm_t, col1 + 55, r3);
    drawValIfChanged(p_ac_dc_bus, ac_dc_bus, col2 + 75, r3);
    drawValIfChanged(p_ac_eev, ac_eev, col3 + 50, r3);

    drawValIfChanged(p_ac_fan, ac_fan, col1 + 60, r4);
    drawValIfChanged(p_ac_press_l, ac_press_l, col2 + 90, r4);
    drawValIfChanged(p_ac_press_h, ac_press_h, col3 + 90, r4);
    tft.setTextPadding(0);

    tft.setTextColor(errColor, headerBg);
    tft.setTextPadding(280);
    drawValIfChanged(p_ac_error_code, ac_error_code, 220, 229);
    tft.setTextPadding(0);
  } 
  else { 
    tft.setTextColor(valColor, bgColor);
    ...
    tft.setTextPadding(75);
    drawValIfChanged(p_ac_status, ac_status, col1 + 90, or1);
    drawValIfChanged(p_ac_freq, ac_freq, col2 + 85, or1);
    drawValIfChanged(p_ac_amp, ac_amp, col3 + 80, or1);

    drawValIfChanged(p_ac_out_t, ac_out_t, col1 + 110, or2);
    drawValIfChanged(p_ac_coil, ac_coil, col2 + 70, or2);
    drawValIfChanged(p_ac_disch, ac_disch, col3 + 70, or2);

    drawValIfChanged(p_ac_ipm_t, ac_ipm_t, col1 + 55, or3);
    drawValIfChanged(p_ac_dc_bus, ac_dc_bus, col2 + 75, or3);
    drawValIfChanged(p_ac_eev, ac_eev, col3 + 50, or3);

    drawValIfChanged(p_ac_fan, ac_fan, col1 + 60, or4);
    drawValIfChanged(p_ac_press_l, ac_press_l, col2 + 90, or4);
    drawValIfChanged(p_ac_press_h, ac_press_h, col3 + 90, or4);

    int ir1 = 220, ir2 = 250;
    drawValIfChanged(p_ac_in_status, ac_in_status, col1 + 90, ir1);
    drawValIfChanged(p_ac_in_model, ac_in_model, col2 + 60, ir1);
    drawValIfChanged(p_ac_in_set, ac_in_set, col3 + 100, ir1);

    drawValIfChanged(p_ac_in_room, ac_in_room, col1 + 55, ir2);
    drawValIfChanged(p_ac_in_coil, ac_in_coil, col2 + 70, ir2);
    drawValIfChanged(p_ac_in_fan, ac_in_fan, col3 + 60, ir2);
    tft.setTextPadding(0);

    tft.setTextColor(errColor, headerBg);
    tft.setTextPadding(280);
    drawValIfChanged(p_ac_error_code, ac_error_code, 220, 285);
    tft.setTextPadding(0);
  }
}
...
void drawScopeScreen(int brandIndex) {
  ...
  draw3DButton(8, 260, 80, 50, btnBg, COLOR_BG, "กลับ", false, false);
  ...
  draw3DButton(392, 260, 80, 50, isScopeRunning ? tft.color565(0, 180, 0) : tft.color565(180, 0, 0), COLOR_BG, isScopeRunning ? "ทำงาน" : "หยุด", false, false);
